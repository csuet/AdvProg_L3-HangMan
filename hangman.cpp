...
bool isCharInWord(const char ch, const string& word)
{
    ...
    for (int i = 0; i < word.length(); i++) if (word[i] == ch) return true;
    return false;
}
...
string chooseWordFromList(const vector<string>& wordList, int index)
{
    ...
    for (int i = 0; i < wordList[index].length(); i++) {
        if ('A' <= wordList[index][i] && wordList[index][i] <= 'Z') answer += char(wordList[index][i] + 32);
        else if ('a' <= wordList[index][i] && wordList[index][i] <= 'z') answer += char(wordList[index][i]);
    }
