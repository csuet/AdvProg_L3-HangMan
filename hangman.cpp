...
{
    if(isCharInWord){
        secretWord = updateSecretWord(secretWord, ch, word);
        correctChars = updateEnteredChars(ch, correctChars)
    }else {
        incorrectGuess = updateIncorrectGuess(incorrectGuess);
        incorrectChars = updateEnteredChars(ch, incorrectChars);
    }
    /*** TODO
        If ch in word:
